  ...
  return (
    <div style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px 12px" }}>
        {[
          ...
          <div key={String(label)} style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ fontSize: 8, color: "rgba(255,255,255,0.3)", fontFamily: "JetBrains Mono", width: 72 }}>
              {label}
            </span>
            <div style={{ flex: 1, height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
              <div style={{
                width: `${val}%`, height: "100%", borderRadius: 2,
                background: Number(val) >= 80 ? "#22d3ee" : Number(val) >= 60 ? "#a78bfa" : Number(val) >= 40 ? "#f59e0b" : "#ef4444",
              }} />
            </div>
            <span style={{ fontSize: 8, color: "rgba(255,255,255,0.3)", fontFamily: "JetBrains Mono", width: 22, textAlign: "right" }}>
              {val}
            </span>
          </div>
        ))}
      ...
      <div style={{ display: "flex", gap: 3, flexWrap: "wrap", marginTop: 5 }}>
        {channel.specialties.slice(0, 4).map(s => (
          <span key={s} style={{
            ...
            background: "rgba(34,211,238,0.06)", color: "#22d3ee", border: "1px solid rgba(34,211,238,0.12)",
          }}>
            {s.replace(/_/g, " ")}
          </span>
        ))}
      ...
      <div style={{ marginTop: 4, fontSize: 9, color: "rgba(255,255,255,0.3)", fontFamily: "JetBrains Mono" }}>
        AVANCE: {Math.abs(channel.medianLeadTimeMinutes)}min {channel.medianLeadTimeMinutes < 0 ? "avant" : "après"} MSM
