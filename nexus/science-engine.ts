 ...
 * Source: ArXiv 2505.09852 — LLMs + RAG améliorent prédiction de 34%
 * vs connaissance paramétrique seule
 *
 * En production: requête ACLED API + GDELT 2.0 pour les 30 derniers jours
 * En démo: données synthétiques calibrées sur historiques réels
 */
export async function buildRAGContext(
  zone: string, country: string, lat: number, lng: number,
  radiusKm = 200, windowDays = 30,
): Promise<RAGContext> {
  // En production: fetch ACLED API + GDELT GKG
  // https://api.acleddata.com/acled/read/?key={KEY}&lat={lat}&lng={lng}&radius={r}&event_date={date_range}
  // https://api.gdeltproject.org/api/v2/doc/doc?query={zone}&mode=artlist&timespan={days}d

  // Données de contexte synthétiques calibrées par zone
  const zoneContext: Record<string, { events: ACLEDEvent[]; trend: number; actors: string[]; drivers: string[] }> = {
    "IL": {
      events: [
        { date: "2026-03-05", event_type: "Air/drone strike", actor1: "IDF", actor2: "Hamas", admin1: "Gaza", country: "PS", fatalities: 12, latitude: 31.5, longitude: 34.45, notes: "IDF strikes Hamas command center north Gaza", geo_precision: 1 },
        { date: "2026-03-04", event_type: "Shelling/artillery", actor1: "Hamas", actor2: "IDF", admin1: "Tel Aviv", country: "IL", fatalities: 0, latitude: 32.08, longitude: 34.78, notes: "Rocket salvo intercepted by Iron Dome, 12 units fired", geo_precision: 2 },
        { date: "2026-03-03", event_type: "Battle", actor1: "IDF", actor2: "Hamas", admin1: "Khan Younis", country: "PS", fatalities: 8, latitude: 31.35, longitude: 34.31, notes: "Ground engagement IDF forces vs Hamas tunnels", geo_precision: 1 },
      ],
      trend: 0.85, actors: ["IDF", "Hamas", "Islamic Jihad", "Hezbollah", "IRGC Quds Force"],
      drivers: ["Gaza blockade", "West Bank settlements", "Jerusalem status", "Hamas governance", "Egyptian mediation failure"],
    },
    "UA": {
      events: [
        { date: "2026-03-05", event_type: "Shelling/artillery", actor1: "Russian Armed Forces", actor2: "Ukrainian Armed Forces", admin1: "Zaporizhzhia", country: "UA", fatalities: 6, latitude: 47.8, longitude: 35.2, notes: "Russian artillery barrage on Ukrainian positions", geo_precision: 1 },
        { date: "2026-03-04", event_type: "Air/drone strike", actor1: "Ukrainian Armed Forces", actor2: "Russian Armed Forces", admin1: "Kharkiv", country: "UA", fatalities: 3, latitude: 49.99, longitude: 36.23, notes: "Ukrainian drone strike on Russian logistics depot", geo_precision: 2 },
      ],
      trend: 0.80, actors: ["Russian Armed Forces", "Ukrainian Armed Forces", "Wagner PMC", "National Guard Ukraine"],
      drivers: ["Territorial control Donbas", "NATO weapons supply", "Crimea bridge strategic target", "Russian mobilization", "Western economic sanctions"],
    },
    "YE": {
      events: [
        { date: "2026-03-05", event_type: "Remote explosive/IED", actor1: "Houthi movement", actor2: "Cargo vessel", admin1: "Red Sea", country: "YE", fatalities: 0, latitude: 15.0, longitude: 43.0, notes: "Houthi anti-ship missile attack on Maersk cargo, ship diverted", geo_precision: 3 },
        { date: "2026-03-04", event_type: "Air/drone strike", actor1: "US/UK Coalition", actor2: "Houthi movement", admin1: "Hodeida", country: "YE", fatalities: 2, latitude: 14.8, longitude: 42.95, notes: "Coalition strike on Houthi weapons storage Hodeida port", geo_precision: 2 },
      ],
      trend: 0.65, actors: ["Houthi movement", "Saudi-led Coalition", "US Navy 5th Fleet", "UK Royal Navy"],
      drivers: ["Gaza solidarity operations", "Red Sea shipping disruption", "Saudi-Iran normalization", "UN peace process stalled"],
    },
  };

  const ctx = zoneContext[country] || {
    events: [],
    trend: 0.40,
    actors: ["Unknown armed actors"],
    drivers: ["Regional tensions"],
  };

  // Calcul du score de violence baseline (ACLED methodology)
  const fatalitiesTotal = ctx.events.reduce((s, e) => s + e.fatalities, 0);
  const baselineViolenceScore = Math.min(1, (ctx.events.length * 0.1 + fatalitiesTotal * 0.01));

  // Évènements GDELT synthétiques (Goldstein scale)
  const gdeltEvents: GDELTEvent[] = [
    {
      date: new Date(Date.now() - 86400000).toISOString().slice(0, 10),
      actor1: "GOVERNMENT", actor2: "MILITARY",
      eventCode: "190", eventDescription: "Use conventional military force",
      goldsteinScale: -9.0, numMentions: 450, numArticles: 89,
      latitude: lat, longitude: lng, avgTone: -6.8,
    },
    {
      date: new Date(Date.now() - 2 * 86400000).toISOString().slice(0, 10),
      actor1: "REBEL", actor2: "CIVILIAN",
      eventCode: "182", eventDescription: "Physically assault",
      goldsteinScale: -7.0, numMentions: 230, numArticles: 45,
      latitude: lat + 0.5, longitude: lng + 0.3, avgTone: -5.2,
    },
  ];

  const escalationTrend = ctx.trend;
  const contextSummary = `${zone} (${country}): ${ctx.events.length} événements ACLED documentés sur ${windowDays}j. ` +
    `Acteurs principaux: ${ctx.actors.slice(0, 3).join(", ")}. ` +
    `Facteurs structurels: ${ctx.drivers.slice(0, 2).join(", ")}. ` +
    `Tendance d'escalade: ${(escalationTrend * 100).toFixed(0)}%. ` +
    `GDELT Goldstein moyen: ${gdeltEvents.reduce((s, e) => s + e.goldsteinScale, 0) / gdeltEvents.length}.`;

  return {
    zone, timeWindowDays: windowDays,
    acledEvents: ctx.events,
    gdeltEvents,
    baselineViolenceScore,
    escalationTrend,
    topActors: ctx.actors,
    conflictDrivers: ctx.drivers,
    contextSummary,
  };
}
