  ...
  const fallback: AbsenceZone[] = [
    {
      ...
      confidenceScore: 0.94, dropPercent: 91, durationMin: 30,
      militaryContext: true, nexusSignalStrength: 0.94,
      baselineTraffic: 140, currentTraffic: 12,
    },
  ];

  const allZones = [...voids, ...darks, ...shutdowns];
  cachedZones = allZones.length > 0 ? allZones : fallback;
  cacheTs = now;
