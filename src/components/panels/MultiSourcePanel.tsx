...
interface SourceStatus {
  id: string;
  status: "LIVE" | "DEMO" | "OFFLINE" | "PENDING";
...
function SourceCard({ src, status, onSelect }: { src: DataSourceConfig; status?: SourceStatus; onSelect: () => void; key?: string }) {
  ...
  const st = status?.status ?? (src.free && !src.envVar ? "DEMO" : "OFFLINE");
  const stColor = st === "LIVE" ? "#4ade80" : st === "DEMO" ? "#f59e0b" : "#ef4444";
  return (
    <div
      onClick={onSelect}
      style={{
...
export function MultiSourcePanel() {
  ...
  useEffect(() => {
    const init: SourceStatus[] = NEXUS_SOURCES.map(s => ({
      id: s.id,
      status: (s.free && !s.envVar ? "DEMO" : s.envVar ? "PENDING" : "OFFLINE") as SourceStatus["status"],
      signalsLast5min: 0,
      category: s.category,
    }));
    setStatuses(init);
        ...
        {tab === "SOURCES" && (
          <>
            <div style={{ fontSize: 8, color: "#475569", fontFamily: "JetBrains Mono,monospace", marginBottom: 7 }}>
              {NEXUS_SOURCES.filter(s => s.free && !s.envVar).length} zero-config · {NEXUS_SOURCES.filter(s => s.free).length} free · {NEXUS_SOURCES.length} total
            </div>
