...
const MOCK_ALERTS: NexusAlert[] = [
  {
    ...
    signals: [
      { icon: "ADS-B", text: "12 avions militaires US B-52/F-35 → Méd. Est ADS-B", source: "aviation" },
      { icon: "X/SOC", text: "+847% tweets Tel Aviv — alerte sirènes / 14,200 shares", source: "social_x" },
      { icon: "TKTK", text: "3 vidéos explosions TikTok CV-confirmées (smoke+crowd)", source: "social_tiktok" },
      { icon: "TG", text: "4 canaux Telegram IDF + sécurité actifs — 340 msgs/h", source: "social_telegram" },
      { icon: "AIS", text: "Navires déroutés port Haïfa — AIS position change ×12", source: "maritime" },
      { icon: "GPSJ", text: "GPS dégradé 180km rayon — brouillage EW IDF confirmé", source: "gpsjam" },
      { icon: "SAT", text: "KH-11 USA-245 + Gaofen-3 stackent zone — BDA probable", source: "satellite" },
      { icon: "NLGT", text: "NASA VIIRS — zones dark détectées secteur nord Gaza", source: "nightlights" },
    ],
    confidence: 94,
  ...
  },
  {
    ...
    signals: [
      { icon: "AIS", text: "USS Ronald Reagan + 3 destroyers repositionnés détroit", source: "maritime" },
      { icon: "X/SOC", text: "Weibo +340% termes militaires (avant fenêtre censure)", source: "social_weibo" },
      { icon: "ADS-B", text: "Avions PLA J-20 décollés base Fujian — cap Taiwan", source: "aviation" },
      { icon: "MKT+", text: "Pétrole +5.2% · actions défense +8.9% simultanés", source: "economic_defense" },
      { icon: "SAT", text: "BARS-M + Gaofen-3 doubles passages détroit en 4h", source: "satellite" },
    ],
    confidence: 81,
  ...
  },
  {
    ...
    signals: [
      { icon: "TG", text: "Canaux Telegram Wagner réactivés — 12 messages chiffrés", source: "social_telegram" },
      { icon: "SAT", text: "Convois 12 véhicules route Tombouctou — Pleiades", source: "satellite" },
      { icon: "FIRM", text: "NASA FIRMS 6 points incendies anormaux route N1", source: "nasa_firms" },
      { icon: "ADS-B", text: "Absence ADS-B rayon 300km — espace aérien fermé implicite", source: "absence_ads_b" },
    ],
    confidence: 73,
    similarEvent: null,
  ...
  },
  {
    ...
    signals: [
      { icon: "ADS-B", text: "Jets privés oligarques +12 décollages anormaux 6h", source: "private_jets" },
      { icon: "FX", text: "Or +2.8% · BTC +4.1% simultanés — fuite capitaux", source: "economic_gold" },
      { icon: "X/SOC", text: "VK activité diplomatique inhabituelle — 890 mentions", source: "social_vk" },
    ],
    confidence: 68,
  ...
  },
  {
    ...
    signals: [
      { icon: "AIS", text: "AIS dark — 3 cargo désactivent transpondeurs zone sud", source: "absence_ais" },
      { icon: "TG", text: "Telegram Houthis — annonce frappe imminente vessels", source: "social_telegram" },
      { icon: "MKT+", text: "BDI -18% semaine — déroutages Suez +23% confirmés", source: "economic_bdi" },
    ],
    confidence: 61,
...
const MOCK_LIVE_SIGNALS: LiveSignal[] = [
  { id: "ls-1", source: "ADS-B", icon: "ADS-B", text: "B-52H — Dyess AFB décollage non planifié", zone: "Texas", confidence: 0.91, timestamp: ago(90000), level: 7 },
  { id: "ls-2", source: "Telegram", icon: "TG", text: "Canal @IDF_Updates: « מצב חירום »", zone: "Tel Aviv", confidence: 0.88, timestamp: ago(95000), level: 8 },
  { id: "ls-3", source: "GPS Jam", icon: "GPSJ", text: "NAC dégradé 47 aéronefs — Gaza-Liban rayon 180km", zone: "Liban", confidence: 0.92, timestamp: ago(110000), level: 7 },
  { id: "ls-4", source: "Twitter/X", icon: "X/SOC", text: "+847% mentions 'explosion' Tel Aviv en 4min", zone: "Tel Aviv", confidence: 0.85, timestamp: ago(125000), level: 8 },
  { id: "ls-5", source: "NORAD TLE", icon: "SAT", text: "KH-11 USA-245 survol zone frappe +2 passages", zone: "Irak/Iran", confidence: 0.95, timestamp: ago(140000), level: 6 },
  { id: "ls-6", source: "NASA VIIRS", icon: "NLGT", text: "Zone dark 340km² — secteur nord Gaza", zone: "Gaza", confidence: 0.87, timestamp: ago(160000), level: 7 },
  { id: "ls-7", source: "AIS", icon: "AIS", text: "MAERSK SENDAI AIS dark — Mer Rouge zone Houthis", zone: "Mer Rouge", confidence: 0.88, timestamp: ago(200000), level: 5 },
  { id: "ls-8", source: "GDELT", icon: "GDLT", text: "GDELT surge: 'strike' 'Israel' 4200 articles/15min", zone: "Global", confidence: 0.75, timestamp: ago(240000), level: 6 },
  { id: "ls-9", source: "ADS-B Absence", icon: "VOID", text: "Void ADS-B 340km³ — Gulf approach corridor", zone: "Golfe Persique", confidence: 0.94, timestamp: ago(270000), level: 8 },
  { id: "ls-10", source: "Brent", icon: "BRNT", text: "Brent spike +$11.4 en 8min — achat panique", zone: "Marchés", confidence: 0.88, timestamp: ago(310000), level: 7 },
  { id: "ls-11", source: "TikTok CV", icon: "TKTK", text: "CV smoke+military detected — 3 vidéos géolocalisées", zone: "Tel Aviv", confidence: 0.84, timestamp: ago(350000), level: 7 },
  { id: "ls-12", source: "Jets Privés", icon: "ADS-B", text: "Abramovich G650ER Dubai → Dubaï → Inconnu", zone: "Moscou", confidence: 0.71, timestamp: ago(420000), level: 4 },
];
...
const MOCK_SOCIAL = [
  ...
  { platform: "Telegram",   icon: "ADS-B", volume: 1240, delta: "+340%", hot: true,  trend: [100,150,200,300,450,600,750,900,1100,1240] },
